  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <nav className="w-full max-w-5xl mx-auto flex items-center justify-between px-6 py-5">
        ...
        <Link
          href="/generate"
          className="px-4 py-2 rounded-xl bg-purple-700/30 hover:bg-purple-600/40 border border-purple-600/30 hover:border-purple-500/50 text-purple-200 text-sm font-medium transition-all"
        >
          Open App
        </Link>
      </nav>
      ...
      <section className="flex-1 flex flex-col items-center justify-center text-center px-6 pt-16 pb-24">
        {/* Badge */}
        <div className="mb-6 inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-purple-900/30 border border-purple-700/30 text-xs text-purple-300/70">
        ...
        <p className="mt-6 text-purple-300/60 text-lg max-w-md leading-relaxed">
        ...
        <div className="mt-10 flex items-center gap-4 flex-wrap justify-center">
          ...
          <span className="text-purple-400/40 text-sm">Free · No sign-up needed</span>
        </div>
        ...
        <div className="mt-16 flex flex-wrap justify-center gap-3 max-w-lg">
          {[
            ...
            <span
              key={text}
              className="px-3 py-1.5 rounded-lg bg-purple-950/50 border border-purple-800/30 text-purple-300/60 text-xs font-mono"
            >
              &ldquo;{text}&rdquo;
            </span>
          ))}
      ...
      <section className="w-full max-w-5xl mx-auto px-6 py-24">
        <h2 className="text-center text-2xl font-bold text-purple-200/80 mb-12">How it works</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {STEPS.map(step => (
            <div key={step.number} className="bg-[#110820] border border-purple-800/20 rounded-2xl p-5 space-y-3">
              <span className="text-3xl font-bold text-purple-800/60 font-mono">{step.number}</span>
              <h3 className="text-purple-200 font-semibold text-sm">{step.title}</h3>
              <p className="text-purple-400/60 text-xs leading-relaxed">{step.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="w-full max-w-5xl mx-auto px-6 pb-24">
        <h2 className="text-center text-2xl font-bold text-purple-200/80 mb-12">Built for your audience</h2>
          ...
          {AUDIENCES.map(a => (
            <div key={a.title} className="bg-[#110820] border border-purple-800/20 rounded-2xl p-6 space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                {a.examples.map(ex => (
                  <span key={ex} className="px-2.5 py-1 rounded-lg bg-purple-900/30 border border-purple-700/20 text-purple-300/70 text-xs font-mono">
                    {ex}
                  </span>
                ))}
              </div>
      ...
      {/* Footer */}
      <footer className="text-center text-purple-400/30 text-xs pb-8">
